...
/** Monorepo libs whose exact/`file:` pins should track latest npm before build. */
...
export async function main(argv = process.argv.slice(2)) {
  ...
  function rewriteFileDepsToNpm() {
    ...
    for (const section of DEP_SECTIONS) {
      ...
      for (const [name, spec] of Object.entries(deps)) {
        ...
        rewritten.push({ name, spec: publishedSpec, section });
        changed = true;
      }
    }
  ...
  function installPublishedSiblings(rewritten) {
    for (const { name, spec, section } of rewritten) {
      const saveFlag =
        section === 'devDependencies'
          ...
          : section === 'peerDependencies'
            ...
            : section === 'optionalDependencies'
              ...
              : '--save';
      run(
        `npm install ${name}@${spec} ${saveFlag} --ignore-scripts --no-audit --no-fund`,
      );
    }
  }
  ...
  function syncSiblingExactPins() {
    const pkgPath = join(root, 'package.json');
    const current = readPackage();
    let changed = false;
    /** @type {{ name: string, spec: string, section: string }[]} */
    const rewritten = [];
    ...
    for (const section of DEP_SECTIONS) {
      ...
      for (const name of SIBLING_PACKAGES) {
        ...
        const isFileDep = spec.startsWith('file:');
        const isExactPin = /^\d/.test(spec);
        if (!isFileDep && !isExactPin) {
          continue;
        }
        let latest;
        try {
          latest = runCapture(`npm view ${name} version`);
        } catch {
          console.warn(
            `publish-ci: could not resolve latest ${name} on npm; leaving ${spec}`,
          );
          continue;
        }
        if (spec === latest) {
        ...
        console.log(`publish-ci: sync sibling ${name} ${spec} → ${latest}`);
        deps[name] = latest;
        rewritten.push({ name, spec: latest, section });
        changed = true;
      }
    }
  ...
  function installDependencies() {
    const { changed: siblingPinsChanged, rewritten: siblingRewritten } =
      syncSiblingExactPins();
    ...
    if (hasLocal) {
      ...
      const { changed, rewritten } = rewriteFileDepsToNpm();
      const allRewritten = [...siblingRewritten, ...rewritten];
      if (siblingPinsChanged || changed) {
        installPublishedSiblings(allRewritten);
      } else {
    ...
    if (siblingPinsChanged) {
      console.log(
        'publish-ci: sibling pins updated; refreshing lockfile via npm install',
      );
      installPublishedSiblings(siblingRewritten);
      ensureGitHubDepsBuilt();
      return;
    }
